...
- **换屏/改分辨率自动纠偏**：识别到显示器尺寸变化（外接屏插拔、分辨率/缩放调整、换回笔记本内置屏）后 0.3 秒内自动切回合适的预设位置，时钟不再偏移到屏外或错位
...
90 条测试覆盖时间/日期格式化、设置迁移与安全读写、字体加载、三种显示方式窗口标志、拖动锁、屏幕位置九宫格与坐标持久化、换屏/改分辨率自动纠偏与预设反推、拖动落点提交、开机自启动命令构造、设置面板预览等。
...
main.py            主程序（窗口、三显示方式、九宫格屏幕位置、换屏自动纠偏、托盘、设置面板、selftest）
tests/             pytest 测试（90 条）
