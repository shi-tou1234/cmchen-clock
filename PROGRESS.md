...
- 收尾：提交 6009afb 推送并打 v1.0.5 tag → Release 工作流 Windows 成功、mac/linux 在 Run tests 失败——`installer_common.py` 模块顶部 `from ctypes import wintypes` 只在 Windows 存在，非 Windows 平台测试收集即 ImportError；改为 ctypes 基础类型（DWORD=c_ulong/HANDLE·HWND=c_void_p/BOOL=c_int/LPARAM=c_ssize_t/UINT=c_uint/WPARAM=c_void_p）后本地 136 全绿＋屏蔽 ctypes.wintypes 模拟非 Windows 收集通过。本地重装 v1.0.6 时又暴露 `PostMessageW` 的 LPARAM 传 None 触发 ctypes ArgumentError（**升级时时钟正在运行必踩**，前几轮测试应用都没在跑故没覆盖）——改传 0 并把向导/卸载窗口的异常捕获放宽为 Exception。按「不覆盖已有 tag」规则升 v1.0.6 重发。
